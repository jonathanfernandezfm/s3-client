  ...
  try {
    ...
    if (successfulResults.length > 0) {
      ...
      if (!activityResult1.ok) {
        console.error("[activity] copy-route lost audit row (source)", { connectionId: sourceConnectionId, bucket: sourceBucket, reason: activityResult1.reason });
      }
      if (targetConnectionId !== sourceConnectionId) {
        ...
        if (!activityResult2.ok) {
          console.error("[activity] copy-route lost audit row (target)", { connectionId: targetConnectionId, bucket: targetBucket, reason: activityResult2.reason });
        }
      }
    }
